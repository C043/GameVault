...
public class GamesController {
    ...
    public ResponseEntity<String> getGames(
        ...
        if (allParams != null && allParams.get("platforms") == null &&
                allParams.get("genres") == null)
            return restTemplate.getForEntity(
                    this.url + "/games?key=" + this.key +
                            "&search=" + allParams.getOrDefault("search", "") +
                            "&page=" + allParams.getOrDefault("page", "1") +
                            "&ordering=" +
                            allParams.getOrDefault("ordering", ""),
                    String.class);
        if (allParams != null && allParams.get("platforms") != null &&
                allParams.get("genres") == null)
            return restTemplate.getForEntity(
    ...
    public ResponseEntity<String> getGameById(@PathVariable String gameId) {
        return restTemplate.getForEntity(
                this.url + "/games/" + gameId + "?key=" + this.key,
                String.class);
    }
}
